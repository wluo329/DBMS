...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        List<Integer> tableIndexes = getEligibleIndexColumns(table);
        for (int i : tableIndexes) {
            SelectPredicate p = selectPredicates.get(i);
            queryOp = new IndexScanOperator(this.transaction, table,
                    p.column, p.operator, p.value);

            //If we find a query plan with lower cost, replace minOp
            if (queryOp.estimateIOCost() < minOp.estimateIOCost()) {
                minOp = queryOp;
                except = i;
            }
        }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> set : prevMap.keySet()) {
            for (JoinPredicate p : this.joinPredicates) {
                String LT = p.leftTable;
                String RT = p.rightTable;
                String LC = p.leftColumn;
                String RC = p.rightColumn;

                if (set.contains(LT) && set.contains(RT)) {
                    continue;
                }

                QueryOperator pass1OP;
                QueryOperator cheapestJoin;

                Set<String> setKey = new HashSet<>(set);

                //Case 1
                if (set.contains(LT) && !set.contains(RT)) {
                    setKey.add(RT);
                    pass1OP = pass1Map.get(new HashSet<>(Arrays.asList(RT)));

                    cheapestJoin = minCostJoinType(prevMap.get(set), pass1OP, LC, RC);

                }
                //Case 2
                else if ((set.contains(RT) && !set.contains(LT))) {
                    setKey.add(LT);
                    pass1OP = pass1Map.get(new HashSet<>(Arrays.asList(LT)));

                    cheapestJoin = minCostJoinType(prevMap.get(set), pass1OP, RC, LC);
                }
                //Case 3
                else {
                    continue;
                ...
                //Case 1/2: Update result map if needed
                if (!result.containsKey(setKey)) {
                    result.put(setKey, cheapestJoin);
                }
                else {
                    // Replace cheaper query
                    if (cheapestJoin.estimateIOCost() < result.get(setKey).estimateIOCost()) {
                        result.put(setKey, cheapestJoin);
                    }
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> lowestCostOperator = new HashMap<>();


        //Pass 1
        for (String t : tableNames) {
            //get lowest cost operator
            QueryOperator val = minCostSingleAccess(t);
            lowestCostOperator.put(new HashSet<>(Arrays.asList(t)), val);
        }

        Map<Set<String>, QueryOperator> prevPass = lowestCostOperator;

        int totalJoins = tableNames.size() - 1;
        //Pass i
        for (int i = 0; i < totalJoins; i++) {
            prevPass = minCostJoins(prevPass, lowestCostOperator);
        }

        //lowest cost operator from last pass
